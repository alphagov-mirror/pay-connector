...
import static uk.gov.pay.connector.util.TestTemplateResourceLoader.WORLDPAY_VALID_AUTHORISE_WORLDPAY_REQUEST_INCLUDING_3DS_NO_SHOPPER_IP_ADDRESS;
...
public class WorldpayPaymentProviderTest extends WorldpayBasePaymentProviderTest {
    ...
    @Test
    public void shouldInclude3dsElementsWithoutShopperIpAddressWhen3dsToggleEnabled() throws Exception {
        ChargeEntity chargeEntity = chargeEntityFixture
                .withExternalId("uniqueSessionId")
                .withAmount(500L)
                .withDescription("This is a description")
        ...
        ArgumentCaptor<GatewayOrder> gatewayOrderArgumentCaptor = ArgumentCaptor.forClass(GatewayOrder.class);
        verify(mockGatewayClient).postRequestFor(eq(WORLDPAY_URL), eq(gatewayAccountEntity), gatewayOrderArgumentCaptor.capture(), anyMap());
        assertXMLEqual(TestTemplateResourceLoader.load(WORLDPAY_VALID_AUTHORISE_WORLDPAY_REQUEST_INCLUDING_3DS_NO_SHOPPER_IP_ADDRESS),
                gatewayOrderArgumentCaptor.getValue().getPayload());
    }
